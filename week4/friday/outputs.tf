...
value = {
  api      = "ssh -i ~/.ssh/kijanikiosk-key.pem ubuntu@${module.app_servers["api"].public_ip}"
  payments = "ssh -i ~/.ssh/kijanikiosk-key.pem ubuntu@${module.app_servers["payments"].public_ip}"
  logs     = "ssh -i ~/.ssh/kijanikiosk-key.pem ubuntu@${module.app_servers["logs"].public_ip}"
}
}